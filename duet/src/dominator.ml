...
module Make (G : Graph.Sig.G) = struct
  ...
  let compute graph init =
    ...
    in
      ...
      (* Traverse vertices in reverse dfs order.  Invariant: eval/link
	 is the forest constructed from the edges of the DFS spanning
	 tree using only vertices with DFS numbers greater than w.  So
	 for any vertex v,
           eval(v) = | v if v < w
	 ...
	 for w = (size - 1) downto 1 do
	   ...
	   in
		...
		note: we're guaranteed that u > w, since we're iterating in
		reverse dfs order. *)
