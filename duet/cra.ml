  ...
  let to_dnf x =
    ...
    let (x_tr, guard, rhs_symbols) =
      BatEnum.fold (fun (x_tr, guard, rhs_symbols) (v, rhs) ->
          ...
          ((v, fresh_rhs)::x_tr,
           (Term.set_expr srk fresh_rhs rhs)::guard,
        ...
        begin match tr_expr (AccessPath (Variable (rhs, OffsetFixed roff))) with
          | TPointer rhs ->
            BatList.reduce K.mul [
              K.arith_assign (VVal lhs) rhs.ptr_val;
              K.arith_assign (VPos lhs) rhs.ptr_pos;
              K.arith_assign (VWidth lhs) rhs.ptr_width;
            ]
          | TInt tint -> begin
              BatList.reduce K.mul [
                K.arith_assign (VVal lhs) tint;
...
let weight def =
  ...
  | Assign (lhs, rhs) ->
    ...
    begin match lhs_typ, rhs_typ with
      ...
      | _, _ -> K.arith_assign (VVal lhs) (tr_expr_val rhs)
    end
  | Store (lhs, rhs) ->
        ...
        begin
          ...
          | OffsetUnknown ->
            Int.Set.fold (fun offset tr ->
                K.add tr (K.arith_assign (VVal (v, OffsetFixed offset)) rhs_val))
              (get_offsets v)
              K.one (* weak update *)
            |> K.mul tr
          | _ ->
            K.mul tr (K.arith_assign (VVal (v,offset)) rhs_val)
        end
      | (MAddr v, offset) ->
  ...
  | Builtin (Alloc (v, size, _)) ->
    BatList.reduce K.mul [
      ...
      K.arith_assign (VWidth v) (tr_expr_val size);
      K.arith_assign (VPos v) (Ctx.mk_real QQ.zero)
    ]
          ...
          if K.mem_transform cost summary then begin
            ...
            let guard =
              ...
              Ctx.mk_and [Syntax.substitute_const srk subst (K.guard summary);
                          Syntax.Term.set_expr srk (Ctx.mk_const cost_symbol) rhs ]
