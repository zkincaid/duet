              ...
              let dta = 
                if !termination_llrf_residual_dta then
                  begin
                    ...
                    let neg_terminating_condition = 
                      Syntax.mk_not srk (TDTA.compute_swf_via_DTA srk exists x_xp residual_formula) in
                    logf "\nneg of DTA precondition for residual:\n%s\n" 
                      (Syntax.Formula.show srk (Quantifier.mbp ~dnf:true srk (fun _ -> true) neg_terminating_condition));
                    ...
                    let subst_condition =
                      ...
                      in
                      Syntax.substitute_const srk subst neg_terminating_condition
                    in
                    logf "\nneg of subst DTA precondition for residual:\n%s\n" (Syntax.Formula.show srk subst_condition);
                    let k_fold_tr_formula = TerminationExp.closure (!K.domain) srk exists x_xp residual_formula in
                    logf "Transitive closure is: %s" (Syntax.Formula.show srk (Quantifier.mbp ~dnf:true srk (fun _ -> true) k_fold_tr_formula));
                    let neg_substed_condition = Syntax.mk_and srk [k_fold_tr_formula; Syntax.mk_not srk subst_condition] in
                    logf "neg of condition to take preimage:\n%s\n" (Syntax.Formula.show srk (Quantifier.mbp ~dnf:true srk (fun _ -> true) neg_substed_condition));
                    ...
                    logf "\nneg of condition generated by LLRF residual+DTA:\n%s" (Syntax.Formula.show srk condition);
                    [condition]
                  end
