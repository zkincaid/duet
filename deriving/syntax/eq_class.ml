...
struct
      ...
      match ts with
        | [t] -> 
            <:module_expr< struct type a = $atype_expr ctxt (`Tuple ts)$;
                                  value eq l r = let module M = $exp ctxt t$ 
                                   in $mexpr$ l r; end >>
        | ts ->
            ...
            in 
              <:module_expr< struct type a = $atype_expr ctxt (`Tuple ts)$;
                                    value eq $Ast.PaTup (loc, lpatt)$ $Ast.PaTup (loc, rpatt)$ = $expr$; end >>
  ...
  method record ?eq ctxt decl fields = 
    if List.exists (function (_,_,`Mutable) -> true | _ -> false) fields then
       <:module_expr< struct type a = $atype ctxt decl$; value eq = (==); end >>
    else
