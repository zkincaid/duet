...
struct
  ...
  let generate ~context ~decls ~make_module_expr ~classname ?default_module () =
    (* plan: 
       ...
       - etc.
    *)
    (* FIXME implicit requirement of classname being equal to module name, hence this hack for Enum *)
    let modulename = if classname = "Enum" then "Deriving_Enum" else classname in
           ...
           if name = "a" then
             ...
             (decl,
              <:module_binding< 
                ...
                : $uid:modulename$.$uid:classname$ with type a = $atype context decl$
               = $apply_defaults (make_module_expr context decl)$ >>))
        decls in
    let sorted_mbinds = make_safe mbinds in
    let mrec =
      <:str_item< open $uid:modulename$; module rec $list:sorted_mbinds$ >> in
      ...
      if tname = "a" then
        ...
        if generated then <:sig_item< >> 
          ...
          let t = List.fold_right 
            ...
            params
            <:module_type< $uid:modulename$.$uid:classname$ with type a = $atype context decl$ >> in
