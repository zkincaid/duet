...
struct
  ...
  struct type a = $atype ctxt decl$;
         value format formatter = fun [ $list:matches$ ]; end >>
    
  let in_a_box box e =
    <:expr< 
      do { Format.$lid:box$ formatter 0;
      $e$;
    ...
    method polycase ctxt : Type.tagspec -> Ast.match_case = function
      ...
      | Tag (name, Some e) ->
          <:match_case< `$uid:name$ x ->
                         $in_hovbox <:expr< 
                            do { Format.pp_print_string formatter $str:"`" ^ name ^" "$;
...
*)
    ...
    method field ctxt : Type.field -> Ast.expr = function
      | (name, ([], t), _) -> <:expr< do { Format.pp_print_string formatter $str:name ^ " ="$;
    ...
    method record ?eq ctxt decl fields = wrap ctxt decl [ <:match_case<
      $record_pattern fields$ -> $in_hovbox
       <:expr<
          do { Format.pp_print_char formatter '{';
               $List.fold_left1
		 (fun l r -> <:expr< do { $l$;
