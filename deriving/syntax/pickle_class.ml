...
struct
  ...
  let unpickle_record ctxt (tname,_,_,_,_ as decl) fields expr = 
    ...
    let assignments = 
      List.fold_right
        (fun (id,_,_) exp ->
           <:expr< do { this.Mutable.$lid:id$ := $lid:id$; $exp$ } >>)
        fields
      <:expr< return self >> in
    let inner = 
      List.fold_right
        (fun (id,(params,t),_) exp ->
           assert (params = []);
  ...
  let wrap ~ctxt ~atype ~tymod ~eqmod ~picklers ~unpickler =
    <:module_expr< struct open Eq; open Typeable;
                          ...
                          type a = $atype$;
                          open Write;
                          value pickle = let module W = Utils(T)(E) in fun [ $list:picklers$ ];
                          open Read;
                          value unpickle = let module W = Utils(T) in $unpickler$;
    end >>
      ...
      let inner = List.fold_right 
        (fun t exp -> <:expr<
           let module M = $(self#expr ctxt t)$ in
             try $exp$
             with [ UnknownTag (n,_) -> (M.unpickle id :> Read.m a) ] >>)
        ts
    ...
    method variant ctxt (tname,_,_,_,_ as decl) (_, tags) = 
      let unpickler = 
        ...
        let extension_case = self#extension ctxt tname extensions in
          <:expr< fun id -> W.sum (fun [ $list:tag_cases @ [extension_case]$ ]) id >>
      in
