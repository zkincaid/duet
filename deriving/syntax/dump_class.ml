...
struct
  ...
  let wrap ~atype ~dumpers ~undump =
    <:module_expr< struct type a = $atype$;
                          value to_buffer buffer = fun [ $list:dumpers$ ];
                          value from_stream stream = $undump$; end >>
  ...
  let instance = object (self)
    ...
    method nargs ctxt (exprs : (name * Type.expr) list) : Ast.expr * Ast.expr =
      List.fold_right
        (fun (id,t) (p,u) -> 
           <:expr< do { $mproject (self#expr ctxt t) "to_buffer"$ buffer $lid:id$; $p$ } >>,
      ...
      let dumpn = <:expr< Dump_int.to_buffer buffer $`int:n$ >> in
        match tagspec with
          | Tag (name, args) -> (match args with 
              ...
              | Some e -> <:match_case< `$name$ x -> do { $dumpn$; 
          ...
          | Extends t -> 
              let patt, guard, cast = cast_pattern ctxt t in
                <:match_case< $patt$ when $guard$ -> 
                               do { $dumpn$; $mproject (self#expr ctxt t) "to_buffer"$ buffer $cast$ } >>,
    ...
    method case ctxt (ctor,args) n =
      match args with 
        ...
        <:match_case< $uid:ctor$ $patt$ -> 
                      do { Dump_int.to_buffer buffer $`int:n$;
                      $dump$ } >>,
