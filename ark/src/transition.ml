...
module Make (Var : Var) = struct
  ...
  module Inner = struct
    type t = tr deriving (Show,Compare)
	...
	let alg = function
	  ...
	  | OVar x -> begin match V.lower x with
		...
		(* v not in env -> v satisfies the recurrence
	   	   v_{k+1} = v_{k}
		*)
		Some (P.zero, T.var x)
	      else begin
...
(*		  Some (px, cx)*)
	...
	*)
    ...
    (* There are two algorithms here: simple_higher_induction_vars, which uses
       ...
       loops within the loop body); and higher_induction_vars, which uses a
       more complicated scheme which is (hopefully) complete. *)
  ...
  include Ka.AddZero(Inner)

  let assign var term =
    Nonzero { transform = M.singleton var term; guard = F.top }

  let assume phi = Nonzero { transform = M.empty; guard = phi }

  let to_smt = function
    | Zero -> Smt.mk_false ()
    | Nonzero tr -> Inner.to_smt tr
  ...
  let simplify = function
    | Zero -> Zero
    | Nonzero x -> Nonzero (Log.time "transform:simplify" Inner.simplify x)
