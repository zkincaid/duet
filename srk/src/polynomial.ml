...
module Ideal = struct
  ...
  let generators ideal = List.map (of_fmon ideal.vs) ideal.basis

  let add_saturate ideal p = 
  ...
  let reduce ideal p =
    Rewrite.reduce (mk_rewrite ideal) p
  ...
  let intersect j k =
    ...
    project (fun i -> i <> t) {basis = gens; vs = (IS.add t vs)}

  let mem p j = 
    let rw = mk_rewrite j in
    Rewrite.reduce_zero rw p

  let subset i j = 
    let rwi, rwj = mk_rewrite i, mk_rewrite j in
  ...
  let pp pp_dim f i = 
    Rewrite.pp pp_dim f (mk_rewrite i)
end
