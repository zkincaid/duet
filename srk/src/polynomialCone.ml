...
let add_polynomial_to_ideal pc p =
  ...
  (Ideal.add_saturate ideal p, cone)
...
let vec_of_multivariate p mono_map =
  let v = V.zero in
...
let project_cone cone_generators f =
  ...
  let vec_generators = BatList.map
      (fun p -> vec_of_multivariate p ordered_mono_map)
      cone_generators
  in
...
let polyhedron_of_cone cone_generators ordered_monomial_map =
  let vec_generators = BatList.map
      (fun p -> vec_of_multivariate p ordered_monomial_map)
      cone_generators
  in
  let generators_enum = BatEnum.map
      (fun v -> (`Ray, v))
...
let mem p pc =
  ...
  let vec_target_poly = vec_of_multivariate reduced ordered_mono_map in
