...
module Model = struct
  ...
  let is_int m p =
    let p' = Polynomial.Rewrite.reduce (PolynomialCone.get_ideal m.cone) p in
...
module Solver = struct
  ...
  let model_of_prop_cube solver prop_cube =
    ...
    let lattice =
      let generators =
        List.map (fst % RR.reduce rewrite % fst) (BatDynArray.to_list int)
      in
      PolynomialConeCpClosure.polylattice_spanned_by generators
    in
