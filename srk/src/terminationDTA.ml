...
let compute_linear_invariants srk pre_symbols_set post_symbols_set inv_symbols_set formula =
...
let compute_best_DLTS_abstraction srk tr_symbols transition_formula =
  let best_dlts = DLTSPeriodicRational.abstract_rational srk tr_symbols transition_formula in
...
let get_coeff_vec_for_expression srk cs expression list_symbols = 
  let vec = 
    BatList.fold_lefti
      (fun existing_coeffs i symbol -> 
         try
           let coeff = get_coeff_of_symbol srk cs expression symbol in
...
module BaseDegPairMap = struct
  ...
  let has_lower_order_than_const base deg =
      ...
      let conditions_list_final, formula_stem_list, encountered_const_order = 
        BatList.fold_left
          (fun (conditions_list, formula_stem, flag) (base, deg, qqt)  ->
             ...
             if has_lower_order_than_const base deg then
...
let generate_term_cond srk cs lhs exp_poly invariant_symbols invariant_terms ineq_type best_DLTS_abstraction x_xp x_set xp_set body_formula =
  let lt_vec, lhs_const = get_coeff_vec_for_expression srk cs lhs invariant_symbols in
...
let analyze_inv_polyhedron srk cs invariants_polyhedron exp_poly invariant_symbols invariant_terms inv_equalities best_DLTS_abstraction x_xp x_set xp_set body_formula =
  let conditions_list = 
    BatList.fold_left
      (fun conditions eq -> 
         let ineqt, lhs = match eq with
           ...
           | `LeqZero t -> Leq0, t
         in 
         let cond = generate_term_cond srk cs lhs exp_poly invariant_symbols invariant_terms ineqt best_DLTS_abstraction x_xp x_set xp_set body_formula in
...
let generate_terminating_DTA_conditions srk tto_transition_formula loop =
  let _, body = loop in
  ...
  | `Sat interp -> 
    ...
    let best_DLTS_abstraction = compute_best_DLTS_abstraction srk x_xp body_formula in 
    ...
    let linear_invariants = compute_linear_invariants srk x_set xp_set inv_terms_set formula in
    ...
    let results_in_inv_terms = analyze_inv_polyhedron srk cs invariants_polyhedron exp_poly invariant_symbols invariant_terms inv_equalities best_DLTS_abstraction x_xp x_set xp_set body_formula in
    ...
    logf "terminating conditions before mbp: %a" (Formula.pp srk) results;
