...
let simple_loop () =
  let query =
    let open Infix in
    mk_query
      [(0, T.arith_assign "x" (int 0), 1);
       (1, T.assume (x < (int 10)), 2);
       (2, T.arith_assign "x" (x + (int 1)), 1);
...
let nested_loop () =
  ...
  let query =
    mk_query
      [(0, T.arith_assign "x" (int 0), 1);
       (1, T.arith_assign "y" (int 0), 2);
       (2, T.assume (x < (int 10)), 3);
       (3, T.arith_assign "z" (int 0), 4);
       (4, T.assume (z < (int 5)), 5);
       (5, T.arith_assign "z" (z + (int 1)), 6);
       (6, T.arith_assign "y" (y + (int 1)), 4);
       (4, T.assume ((int 5) <= z), 7);
       (7, T.arith_assign "x" (x + (int 1)), 2);
  ...
  let query =
    mk_query
      [(0, T.arith_assign "x" (int 0), 1);
       (1, T.assume (x < (int 10)), 2);
       (1, T.assume ((int 10) <= x), 3);
       (10, T.arith_assign "x" (x + (int 1)), 11);
       (10, T.arith_assign "x" (x - (int 1)), 11)]
      [(2, (10, 11), 1)]
      0
  in
...
let recursive () =
  ...
  let query =
    mk_query
      [(0, T.parallel_assign [("x", (int 100 :> Ctx.t Syntax.term)); ("y", (int 0 :> Ctx.t Syntax.term))], 1);
       (10, T.assume ((int 0) < x), 11);
       (10, T.assume (x <= (int 0)), 12);
       (11, T.arith_assign "x" (x - (int 1)), 13);
       (14, T.arith_assign "y" (y + (int 1)), 12)]
      [(1, (10, 12), 2);
       (13, (10, 12),14)]
      0
  in
...
let aff_eq1 () =
  ...
  let ts =
    mk_ts
      [(0, T.parallel_assign [("x", (int 0 :> Ctx.t Syntax.term)); ("y", (int 10 :> Ctx.t Syntax.term))], 1);
       (1, T.assume (x <= (int 10)), 2);
       (2, T.arith_assign "x" (x + (int 1)), 3);
       (3, T.arith_assign "y" (y + (int 1)), 1)]
      []
  in
        ...
        T.parallel_assign
          [("x",
            (mk_ite srk
              (x mod (int 2) = (int 0))
              (x / (int 2))
              ((int 3) * x + (int 1)) :> Ctx.t Syntax.term));
           ("y",
            (mk_ite srk
              (y mod (int 2) = (int 0))
              (y / (int 2))
              ((int 3) * y + (int 1)) :> Ctx.t Syntax.term))],
        1)]
      []
  in
  let inv = affine_invariants ts 0 in
...
let aff_karr_fig4 () =
  ...
  let ts =
    mk_ts
      [
        (0, T.arith_assign "x" (y + (int 1)), 1);
        (0, T.arith_assign "y" (x + (int 1)), 2);
        (1, T.arith_assign "x" (x - (int 2)), 2);
        (2, T.arith_assign "y" (y - (int 2)), 1);
      ]
      []
  in
  let inv = affine_invariants ts 0 in
...
let aff_karr_fig5 () =
  ...
  let ts =
    mk_ts
      [(0,
        T.parallel_assign
          [("x", (int 2 :> Ctx.t Syntax.term));
           ("y", (z + (int 5) :> Ctx.t Syntax.term))],
        1);
       (1,
        T.parallel_assign
          [("x", (x + int 1 :> Ctx.t Syntax.term));
           ("y", (y + (int 3) :> Ctx.t Syntax.term))],
        1)]
      []
  in
  let inv = affine_invariants ts 0 in
...
let suite = "WeightedGraph" >::: [
    ...
    "deep_call" >:: (fun () ->
      ...
      let query =
        mk_query
          [(0, T.arith_assign "x" (int 0), 1);
           (3, T.arith_assign "x" (x + (int 1)), 4);
           (6, T.arith_assign "x" (x + (int 2)), 7);
           (9, T.arith_assign "x" (x + (int 3)), 10)]
          [(1, (3, 5), 2);
           (4, (6, 8), 5);
           (7, (9, 10), 8)]
          0
