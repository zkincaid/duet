...
module Make (A : Alphabet) (P : Predicate) = struct
  ...
  let show_predicate = SrkUtil.mk_show P.pp
  ...
  let pp formatter pa =
    ...
    if PSet.is_empty pa.accepting then
      ...
      Format.fprintf formatter "final: %a.@\n"
        (SrkUtil.pp_print_enum P.pp) (PSet.enum pa.accepting);
  ...
  let pp_ground size formatter pa =
    let open Format in
    let mk_tuples k =
      SrkUtil.tuples (BatList.of_enum ((1 -- k) /@ (fun _ -> (1 -- size))))
    in
    ...
    let (pp_bit_rep, bits) =
      ...
      BatEnum.iter (fun (letter,i) ->
        ...
        ) (SrkUtil.cartesian_product (LetterSet.enum pa.alphabet) (1 -- size));
      let nb_bits =
        let rec lg index n =
    ...
    fprintf formatter "locations = [%a],@\n"
      (SrkUtil.pp_print_enum Format.pp_print_int)
  ...
  let succs pa config (letter, i) =
    ...
    in
    let combine x y =
      SrkUtil.cartesian_product x y /@ (uncurry Config.union)
    in
    try
...
module MakeEmpty (A : sig
  ...
  module Arg = struct
    ...
    module PredicateTree = SearchTree.Make(Config.Predicate)(SrkUtil.Int)
    module FeatureTree = Srk.FeatureTree
